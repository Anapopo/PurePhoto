...
void CPurePhotoDlg::OnBnClickedBtnOpen()
{
	...
	image = cvLoadImage(charPath, CV_LOAD_IMAGE_COLOR); //显示图片

	Mat temp = imread(charPath, CV_LOAD_IMAGE_COLOR);
	temp.copyTo(*currentSrc);
	
	int len = strlen(charPath);    //计算字符串长度  
	currentPicPath = new char[len + 1];    //分配存储空间  
	strcpy_s(currentPicPath, len + 1, charPath); //将s中字符串复制到str，最后一个空间为'\0'结束符  
...
{
	IplImage *image = cvLoadImage("D:\\default.png", 1); //显示图片 
	DrawPicToHDC(image, IDC_CV_PIC);
	if (image) cvReleaseImage(&image);
	//保存至栈区
